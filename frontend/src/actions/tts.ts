...
const S3_BUCKET_URL = env.AWS_S3_BUCKET_URL;

function generateParamsHash(data: GenerateSpeechData, userId: string): string {
...
export async function generateSpeech(
  ...
  try {
    ...
    const paramsHash = generateParamsHash(data, session.user.id);


    // Cache Check: Before calling Modal, check for existing generation with same params
    const cachedProject = await db.audioProject.findFirst({
      where: { paramsHash },
    });

    if (cachedProject) {
      // If found, immediately deduct credits and create a new project record for this user
      // No need to call Modal
      await db.user.update({
        where: { id: session.user.id },
        data: { credits: { decrement: creditsNeeded } },
      });

      const newProject = await db.audioProject.create({
        data: {
          text: data.text,
          audioUrl: cachedProject.audioUrl,
          s3Key: cachedProject.s3Key,
          language: data.language,
